...
#[serde(default)]
pub struct Server {
    /// The protocol to use: "irc" or "veilid".
    #[serde(default)]
    ...
    /// The server to connect to (required for IRC protocol).
    pub server: Option<String>,
    /// The port to connect on (required for IRC protocol).
    pub port: Option<u16>,
    /// The Veilid route blob (base64 encoded) to connect to (required for Veilid protocol).
    pub veilid_route_blob: Option<String>,
...
impl Server {
    pub fn new(
        ...
        Self {
            ...
            nickname,
            server: Some(server),
            port: Some(port.unwrap_or(if use_tls {
                DEFAULT_TLS_PORT
            } else {
                DEFAULT_PORT
            })),
    ...
    ) -> Result<connection::Config<'_>, Error> {
        let server = self.server.as_ref().ok_or_else(|| {
            Error::MissingField(
                "server field is required for IRC protocol".to_string(),
            )
        })?;
        let port = self.port.ok_or_else(|| {
            Error::MissingField(
                "port field is required for IRC protocol".to_string(),
            )
        })?;
    ...
    pub fn veilid_route_blob(&self) -> Result<Vec<u8>, Error> {
        let blob_str = self.veilid_route_blob.as_ref().ok_or_else(|| {
            Error::MissingField(
                "veilid_route_blob field is required for Veilid protocol"
                    .to_string(),
            )
        })?;

        use data_encoding::BASE64;
        BASE64.decode(blob_str.as_bytes()).map_err(|e| {
            Error::InvalidVeilidRouteBlob(format!(
                "Failed to decode base64 route blob: {e}"
            ))
        })
    }
...
impl Default for Server {
    fn default() -> Self {
        Self {
            ...
            server: Some(String::default()),
            port: Some(DEFAULT_TLS_PORT),
            veilid_route_blob: Option::default(),
